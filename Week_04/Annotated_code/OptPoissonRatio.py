...
"""
Computes:
    1. Poisson ratio in a specific direction `θ` for given parameters `T` and strains `E`.
    2. Utilizes the compliance tensor `C_inv` for directional stiffness and contraction.
"""
@tf.function
def computeDirectionalPoissonRatio(n_tiling_params, inputs, thetas, model):
    ...
    # Compute the Voigt representation of directional vectors d and n
    ...
    # Compute energy density, stress, and stiffness tensor C
    psi, stress, C = valueGradHessian(n_tiling_params, inputs, model)
    
    # Compute compliance tensor C⁻¹
    C_inv = tf.linalg.inv(C[0, :, :])

    # Calculate Sd = C⁻¹d and Sn = C⁻¹n for the first element
    Sd = tf.linalg.matvec(C_inv, d_voigt[0, :])
    Sn = tf.linalg.matvec(C_inv, n_voigt[0, :])

    # Compute directional stiffness and Poisson ratio for the first element
    dTSd = tf.expand_dims(tf.tensordot(d_voigt[0, :], Sd, 1), axis=0)
    dTSn = tf.expand_dims(tf.tensordot(d_voigt[0, :], Sn, 1), axis=0)
    nu = -tf.divide(dTSn, dTSd) #poisson ratio
    
    # Repeat computation for all elements in the batch
...
def computePoissonRatio():
    """
    Computes:
        - Poisson ratio for a range of angles `θ` using the provided tiling parameters `T` and strain.

    Purpose:
        - Evaluate how the material's Poisson ratio varies with direction.

    Process:
        - Compute uniaxial strain under directional constraints.
        - Use `computeDirectionalPoissonRatio` to determine the directional Poisson ratio.
    """
    bounds = []
    IH = 21 # Example material ID
    n_sp_theta = 50 # Number of directional samples
    dtheta = np.pi/float(n_sp_theta) # Angle increment
    thetas = np.arange(0.0, np.pi, dtheta)  # Angles spanning [0, π]
    strain = 0.1 # Apply a constant strain
    if strain < 0:
        strain = strain - 0.5 * strain * strain
    else:
        strain = strain + 0.5 * strain * strain

    # Load the material model and parameters
    ...
    # Plot the directional Poisson ratio as a polar plot
...
def getDirectionPoissonRatio(ti, n_tiling_params, model, strain_cauchy, n_sp_theta = 20, sym=True):
    """
    Computes:
        - Directional Poisson ratio for specified material parameters and strain.
        - Given tiling parameters and applied Cauchy strain

    Inputs:
        - ti: Tiling parameters (vector of material design parameters).
        - n_tiling_params: Number of tiling parameters (dimensionality of `ti`).
        - model: Neural network model representing material behavior.
        - strain_cauchy: Applied strain in Cauchy form.
        - n_sp_theta: Number of directional samples (default = 20).
        - sym: Boolean to extend results symmetrically over the range [0, 2π] (default = True).

    Outputs:
        - thetas: Array of angles representing directions.
        - poisson_ratio: Corresponding Poisson ratio values for each direction.

    Links to Paper:
        - Equation (11): Stiffness tensor `C` used to compute directional Poisson ratio.
        - Poisson ratio formula
    """   

    # Convert strain from Cauchy to Green-Lagrange form 
    if strain_cauchy <  0:
        strain = strain_cauchy - 0.5 * strain_cauchy  * strain_cauchy
    else:
        strain = strain_cauchy + 0.5 * strain_cauchy  * strain_cauchy
    
    # Generate directional angles from 0 to π
    thetas = np.arange(0.0, np.pi, np.pi/float(n_sp_theta))

    # Use optimization to compute uniaxial strain for each direction
    uniaxial_strain = computeUniaxialStrainThetaBatch(n_tiling_params, strain, thetas, model, ti, True)
    
    # Prepare inputs for the model by combining tiling parameters and strains
    batch_dim = len(thetas)
    ti_batch = np.tile(ti, (batch_dim, 1))
    nn_inputs = tf.convert_to_tensor(np.hstack((ti_batch, uniaxial_strain)))

    # Compute directional Poisson ratios using stiffness tensor `C`
...
def objGradPoissonRatio(ti, uniaxial_strain, thetas, model):
    """
    Computes:
        - Poisson ratio, its gradient w.r.t tiling parameters `∂ν/∂T`, 
        and its sensitivity to strain `∂ν/∂E`.

    Inputs:
        - ti: Tiling parameters.
        - uniaxial_strain: Applied strains for each direction.
        - thetas: Angles representing material directions.
        - model: Neural network model.

    Outputs:
        - nu: Directional Poisson ratio for all directions.
        - grad: Gradient of Poisson ratio w.r.t tiling parameters `∂ν/∂T`.
        - dOdE: Sensitivity of Poisson ratio to strain `∂ν/∂E`.

    Links to Paper:
        - Equation (11): Used to compute directional Poisson ratio.
        - Gradients and sensitivities are computed to optimize material parameters.
    """    
    batch_dim = uniaxial_strain.shape[0]

    # Generate directional vectors d and n in Voigt notation
    thetas = tf.expand_dims(thetas, axis=1)
    ...
    # Expand tiling parameters for batch processing
    ...
    with tf.GradientTape(persistent=True) as tape_outer_outer:
        ...
        with tf.GradientTape() as tape_outer:
            ...
            with tf.GradientTape(persistent=True) as tape:
                ...
                # Compute energy density and stress
                psi = model(inputs, training=False)
                stress = tape.gradient(psi, uniaxial_strain)
        # Compute stiffness tensor C as ∂S/∂E
        C = tape_outer.batch_jacobian(stress, uniaxial_strain)
        
        # Initialize directional Poisson ratio computation
        C_inv = tf.linalg.inv(C[0, :, :])
        Sd = tf.linalg.matvec(C_inv, d_voigt[0, :])  # Directional stiffness
        Sn = tf.linalg.matvec(C_inv, n_voigt[0, :]) # Lateral contraction
        dTSd = tf.expand_dims(tf.tensordot(d_voigt[0, :], Sd, 1), axis=0)
        dTSn = tf.expand_dims(tf.tensordot(d_voigt[0, :], Sn, 1), axis=0)
        nu = -tf.divide(dTSn, dTSd)

        # Compute for remaining batch elements
        for i in range(1, C.shape[0]):
            C_inv = tf.linalg.inv(C[i, :, :])
            Sd = tf.linalg.matvec(C_inv, d_voigt[i, :])
            Sn = tf.linalg.matvec(C_inv, n_voigt[i, :])
    ...
    # Compute gradients w.r.t. tiling parameters and strain
    grad = tape_outer_outer.jacobian(nu, ti) # ∂ν/∂T
    dOdE = tape_outer_outer.jacobian(nu, uniaxial_strain)  # ∂ν/∂E
    del tape
    del tape_outer
...
def generatePoissonRatioDataThetas(thetas, n_tiling_params, strain, ti, model):
    """
    Generates Poisson ratio data for a range of angles θ.

    Inputs:
        - thetas: Angles representing material directions.
        - n_tiling_params: Number of tiling parameters.
        - strain: Applied strain (Green-Lagrange form).
        - ti: Tiling parameters.
        - model: Neural network model.

    Outputs:
        - poisson_ratio: Directional Poisson ratios for given `θ`.
    """    
    uniaxial_strain = []
    # Compute uniaxial strain for each angle θ
    for theta in thetas:
...
def poissonRatioSA(IH, plot_sim = False, plot_GT = False):
    """
    Performs Poisson ratio optimization for a specific material (identified by IH).

    Inputs:
        - IH: Identifier for the material type/model to optimize.
        - plot_sim: Boolean indicating if simulated results should be plotted (default = False).
        - plot_GT: Boolean indicating if ground truth should be plotted (default = False).

    Outputs:
        - Saves optimization results, plots of Poisson ratio, and optional comparison between initial, optimized, and target values.

    Workflow:
        1. Load the model corresponding to the given IH identifier.
        2. Define material-specific parameters such as bounds, tiling parameters (ti), and strain.
        3. Generate directional Poisson ratios for given angles and perform optimization.
        4. Visualize results (polar plots of Poisson ratio).
    """    
    bounds = []
    
    # Number of directional angles to sample
    n_sp_theta = 50
    thetas = np.arange(0.0, np.pi, np.pi/float(n_sp_theta)) # Angles in radians [0, π].
    strain = 0.01
    current_dir = os.path.dirname(os.path.realpath(__file__))
    idx = np.arange(0, len(thetas), 5)  # Subsampled indices for certain computations.

    # Configure material-specific parameters based on IH
    if IH == 21:

        # for each configuration：
        # target srain, tiling parameter, target tiling parameters
        strain = 0.02
        strain = strain + 0.5 * strain * strain
        n_tiling_params = 2
    ...
    base_dir = "../results/poisson_ratio/"
     ...
     # Optimization objective and gradient computation. x: tiling parameters
    def objAndGradient(x):
        _uniaxial_strain = []
        dqdp = [] # Sensitivity derivatives ∂E/∂T.
        for theta in thetas:
            # for each direction, run INNER optimization using x to get strain tensor with target strain 
            uni_strain, dqidpi = optimizeUniaxialStrainSingleDirectionConstraint(model, n_tiling_params, theta, strain, x, False)
            _uniaxial_strain.append(uni_strain) #concatenate voigt strain tensor
            dqdp.append(dqidpi) #concatenate sensitivity ∂E/∂x at current angle
        ...
        uniaxial_strain_TF = tf.convert_to_tensor(_uniaxial_strain)
         ...
         # Compute Poisson ratio using current x and optimized E, and the gradient w.r.t. x and E
        ...
        # Objective: Least squares between current and target Poisson ratios
        ...
        # Gradient: Total derivative w.r.t. tiling parameters
        grad = np.zeros((n_tiling_params))
        
        for i in range(len(idx)):
            # Compute the residual (error) between the current and target Poisson ratios at angle `i`.
            residual = poisson_ratio_current[i] - poisson_ratio_targets_sub[i]

            # First term: Direct sensitivity of the objective function to the tiling parameters (T).
            # This is the product of the residual and the gradient of the Poisson ratio with respect to the tiling parameters:
            # residual * (∂ν/∂T)
            direct_contribution = residual * poisson_ratio_grad[i].flatten()

            # Second term: Indirect sensitivity via strain (E).
            # This accounts for how changes in tiling parameters (T) affect strain (E),
            # and in turn, how those changes in strain affect the Poisson ratio.
            # Mathematically: residual * (∂ν/∂E) * (∂E/∂T).
            indirect_contribution = residual * np.dot(dOdE[i][i], dqdp[i][:3, :]).flatten()

            # Accumulate the total contribution (direct + indirect) into the gradient.
            grad += direct_contribution + indirect_contribution
    ...
    if (not plot_GT) and (not plot_sim):
        result = minimize(objAndGradient, ti, method='trust-constr', jac=True, options={'disp' : True}, bounds=bounds) # Optimized tiling parameters.
